...
class NVB_OT_amt_amt2psb(bpy.types.Operator):
    ...
    def copy_keyframes(self, amt, amt_action, amb_name, psb_name,
        ...
        def matrix_to_rotmode(mat_list, posebone, to_rotmode):
            if to_rotmode == 'AXIS_ANGLE':
                quats = [m.to_quaternion() for m in mat_list]
                return [[q.angle, *q.axis] for q in quats]
            elif to_rotmode == 'QUATERNION':
                quats = [m.to_quaternion() for m in mat_list]
                return quats
            else:  # Convert to Euler (with filter)
                euls = []
                e = posebone.rotation_euler
                for m in mat_list:
                    e = m.to_euler('XYZ', e)
        ...
        dp_list = get_data_paths(amt_bone.rotation_mode)
        # Transfer keyframes
        for psd_dp, group_name, dp_dim, kfp_convert_space, kfp_dfl in dp_list:
            ...
            if amt_fcu.count(None) < dp_dim:  # disregard empty animations
                ...
                values = kfp_convert_space(amt, amt_bone, psd_bone, values, mat_eb)
                # values = matrix_to_rotmode(values, psd_bone.rotation_mode)
...
class NVB_OT_amt_psb2amt(bpy.types.Operator):
    ...
    def create_bones_rec(self, am, psb, auto_connect=False, strip_name=False,
        ...
        # Create children
        for c in valid_children:
            self.create_bones_rec(am, c, auto_connect, strip_name,
                                  amb, psb_mat)

    def create_bones(self, context, am, psb_root, auto_connect, strip_name):
        bpy.ops.object.mode_set(mode='EDIT')
    ...
    def create_bone_properties(self, context, am):
        ...
        for amb_name, psb, _ in self.generated_bones:
            if psb.type == 'EMPTY':
                am.data.bones[amb_name].nvb.psd_bone_shape = 'EMT'
            else:
                am.data.bones[amb_name].nvb.psd_bone_shape = 'ME1'

    def copy_keyframes(self, am, am_action, amb_name, psb_name,
        ...
        def convert_loc(am, amb, meb, kfvalues, amb_base):
            mats = [mathutils.Matrix.Translation(v) for v in kfvalues]
            mats = [meb.convert_space(pose_bone=None,
                                     matrix=m,
                                     from_space='LOCAL',
                                     to_space='WORLD')
                    for m in mats]
            mats = [am.convert_space(pose_bone=amb,
                                     matrix=m,
                                     from_space='WORLD',
                                     to_space='LOCAL')
                    for m in mats]
        ...
        def convert_axan(am, amb, meb, kfvalues, amb_base):
            """Convert rotation values from mesh space to bone space."""
            # Convert from axis angle to matrices
            mat_list = [mathutils.Quaternion(v[1:], v[0]).to_matrix().to_4x4()
                        for v in kfvalues]
            # Convert from mesh-bone space to armature-bone space
            tm = amb_base.to_4x4()
            tm_inv = amb_base.inverted().to_4x4()
            mat_list = [tm_inv @ mat @ tm for mat in mat_list]
            # Convert back to Axis Angle
            quat_list = [mat.to_quaternion() for mat in mat_list]
            return [[quat.angle, *quat.axis] for quat in quat_list]

        def convert_quat(am, amb, meb, kfvalues, amb_base):
            """Convert rotation values from mesh space to bone space."""
            # Convert keyframe quats to matrices
            mat_list = [cmat @ mathutils.Quaternion(v).to_matrix().to_4x4()
                        for v in kfvalues]
            # Convert from mesh-bone space to armature-bone space
            tm = amb_base.to_4x4()
            tm_inv = amb_base.inverted().to_4x4()
            ...
            return [list(mat.to_quaternion()) for mat in mat_list]

        def convert_eul(am, amb, meb, kfvalues, amb_base):
            """Convert rotation values from mesh space to bone space."""
            # Convert keyframe eulers to matrices
            mat_list = [mathutils.Euler(v, 'XYZ').to_matrix().to_4x4()
                        for v in kfvalues]
            # Convert from mesh-bone space to armature-bone space
            tm = amb_base.to_4x4()
            tm_inv = amb_base.inverted().to_4x4()
            mat_list = [tm_inv @ mat @ tm for mat in mat_list]
            # Convert back to Euler (with filter)
            euls = []
            e = amb.rotation_euler
            for mat in mat_list:
                e = mat.to_euler('XYZ', e)
                euls.append(e)
            return euls

        # This shouldn't happen, but we don't want to crash randomly
        if amb_name not in am.pose.bones:
            return
        # Get posebone and pseudobone (=mesh bone)
        amb = am.pose.bones[amb_name]  # armature bone
        meb = bpy.data.objects[psb_name]  # pseudo bone (a mesh)
        # Get bone base as matrix from bone
        amb_base = mathutils.Matrix([amb.x_axis, amb.y_axis, amb.z_axis])
        amb_base.transpose()
        # Copy rotation mode
        amb.rotation_mode = meb.rotation_mode
        # Copy action
        if meb.animation_data and meb.animation_data.action:
            # list of data paths to convert and their respective dimensions
            ...
            # All fcurves for this action
            meb_fcu = meb.animation_data.action.fcurves
            for dp, dp_dim, dp_convert in dp_list:
                # List of valid curves for this data path
                fcu_list = [meb_fcu.find(data_path=dp, index=i)
                           for i in range(dp_dim)]
                if fcu_list.count(None) < 1:
                    am_dp = 'pose.bones["' + amb_name + '"].' + dp
                    # Get keyed frames
                    if frame_range:  # Grab only frames within range
                        min_frame, max_frame = frame_range
                        frames = list(set().union(
                            *[[k.co[0] for k in fcu_list[i].keyframe_points
                              if min_frame <= k.co[0] <= max_frame]
                              for i in range(dp_dim) if fcu_list[i]]))
                    else: # Grab all frames
                        frames = list(set().union(
                            *[[k.co[0] for k in fcu_list[i].keyframe_points]
                              for i in range(dp_dim) if fcu_list[i]]))
                    # We used a set to remove duplicates, re-sort to be safe
                    frames.sort()
                    # Evaluate fcurve at keyed frames
                    values = [[fcu_list[i].evaluate(f)
                              for i in range(dp_dim)] for f in frames]
                    # Convert from mesh-bone to armature-bone space
                    values = dp_convert(am, amb, meb, values, amb_base)
                    # Create fcurves for armature
                    amt_fcu = [nvb_utils.get_fcurve(am_action, am_dp, i,
                                                    amb_name)
                               for i in range(dp_dim)]
